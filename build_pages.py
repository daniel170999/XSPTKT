...
def build(root: Path, days_limit: int, check: bool) -> int:
    ...
    docs = {
        ...
        "nguon-du-lieu": ("Nguồn dữ liệu | Kết Số", "Nguồn dữ liệu", "<p>Dữ liệu kết quả được tổng hợp từ các nguồn công bố kết quả xổ số kiến thiết, kiểm tra cấu trúc trước khi cập nhật vào kho dữ liệu.</p><p>Mỗi lần cập nhật chỉ bổ sung ngày hợp lệ; dữ liệu cũ không bị thay bằng kết quả rỗng khi nguồn gặp sự cố.</p>"),
